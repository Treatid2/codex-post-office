...
class DatabaseFoundationTests(unittest.TestCase):
    def test_initial_schema_is_transactional_integral_and_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(first["database"]["userVersion"], 3)
            ...
            self.assertEqual(inspect_database(path)["database"]["userVersion"], 3)
    ...
    def test_transactional_backup_is_integral_and_receipted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(inspect_database(backup)["database"]["userVersion"], 3)
    ...
    def test_schema_drift_and_extra_migration_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                con.execute(
                    "INSERT INTO schema_migrations(version,name,sha256,applied_at) VALUES(4,'unknown',?,'2026-09-04T12:00:00Z')",
                    ("f" * 64,),
                )
